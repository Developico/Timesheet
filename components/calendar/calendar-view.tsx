    ...
    return (
      <div className="grid grid-cols-7 gap-2">
        {weekDays.map(day=>{
          ...
          return (
            <div key={day.toISOString()} className={`p-3 h-44 border rounded-lg flex flex-col overflow-hidden transition-colors bg-card ${isWeekend? 'bg-muted/20':''} ${isToday? 'ring-2 ring-[#6eedd9]':''}`}>
      ...
      <div className="grid grid-cols-7 gap-2">
        {days.map((d, i) => {
          ...
          return (
            <div
              key={d}
              className={`p-2 h-28 border rounded-lg transition-colors overflow-hidden flex flex-col bg-card hover:bg-card/95 ${isWeekend? 'bg-muted/20':''} ${isToday? 'ring-2 ring-[#6eedd9]':''}`}
            >
